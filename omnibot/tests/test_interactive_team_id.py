...
MOCK_DIR = os.path.join(os.path.dirname(os.path.dirname(TESTS_DIR)), "data", "mock", "interactive")
...
def test_missing_team_id(mock_get_team, mock_get_bot):
    ...
    with patch('flask.request') as mock_request:
        ...
        assert "No team id found in interactive component" in response[0].get_json()["error"] 