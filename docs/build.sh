...
# We need to set OMNIBOT_DOCS_VERSION_STRING and OMNIBOT_DOCS_RELEASE_LEVEL for Sphinx.
...
then
  ...
  export OMNIBOT_DOCS_VERSION_STRING="tag-$TRAVIS_TAG"
  export OMNIBOT_DOCS_RELEASE_LEVEL=tagged
  export OMNIBOT_BLOB_SHA="$TRAVIS_TAG"
else
  BUILD_SHA=$(git rev-parse HEAD)
  VERSION_NUM=$(cat VERSION)
  export OMNIBOT_DOCS_VERSION_STRING="${VERSION_NUM}"-"${BUILD_SHA:0:6}"
  export OMNIBOT_DOCS_RELEASE_LEVEL=pre-release
  export OMNIBOT_BLOB_SHA="$BUILD_SHA"
fi
...
set -x
sphinx-apidoc -o "${GENERATED_AUTOGEN_RST_DIR}" -T -P ../omnibot
