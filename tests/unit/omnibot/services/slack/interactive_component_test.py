...
def test_interactive_component(mocker):
    ...
    get_user_mock = mocker.patch(
        'omnibot.services.slack.get_user'
    )
    user_ret = {'A12345678': {}}
    get_user_mock.return_value = user_ret
    get_channel_mock = mocker.patch(
        'omnibot.services.slack.get_channel'
    )
    ...
    channels_ret = {
        '<#C123456AB|channel-channel>': 'channel-channel'
    }
    ...
    assert _component.message['parsed_text'] == '@echobot echo I am @here in #channel-channel. See: http://example.com :simple_smile:'  # noqa:E501
